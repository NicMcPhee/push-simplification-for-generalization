...
as well as not opening any new parenthesized code blocks or ending any open parenthesized code blocks.
\todo{I may have lied in this sentence. Can 
	silencing a gene like an \texttt{exec-if} prevent it from opening new blocks?}
When picking a random gene to silence, we never select a gene that is already silent.
...
The four genome methods of simplification are listed in Table \ref{table:genome-methods}. Each variant lists the types of steps that can be taken and the probability of each step being chosen. The most basic of the methods, 
...
\todo[inline]{But doesn't silencing, e.g., something like \texttt{exec-if}
change the parens downstream?}
...
consistently had the highest success rates. 
