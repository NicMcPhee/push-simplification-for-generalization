...
Simplification of evolved programs has been used for various reasons in GP, though as far as we know, never explicitly to improve generalization. Some of the primary uses include making evolved programs more understandable \cite{} and controlling bloat during a run \cite{}. Additionally, algebraic simplification has been used to combat exponential code growth in semantic (crossover?/genetic programming?) \cite{}.
...
An obvious extension of the work presented here would be to run the same tests on programs that solve problems of types other than software synthesis, such as classification and symbolic regression. Another area for future work is to improve the automatic simplification methods themselves. While we presented results with several simplification methods, all of them used a simple hill-climbing search procedure that can easily get stuck in local minima. Even something only slightly more complex, such as a multi-start hillclimber, might simplify programs more reliably, and it would be interesting to see what effects this would have on generalization.
